...
class FakeCriteriaBuilder implements CriteriaBuilder {
    ...
    public Predicate conjunction() {
        return new FakePredicate("1=1");
    }

    @Override
    public Predicate disjunction() {
        return new FakePredicate("1=0");
    }

    @Override
    public Predicate equal(Expression<?> x, Object y) {
        return new FakePredicate(x + " equal " + y);
    }

    @Override
    public Predicate notEqual(Expression<?> x, Object y) {
        return new FakePredicate(x + " not equal " + y);
    }

    @Override
    public Expression<String> upper(Expression<String> x) {
        return new FakeRoot<String>(String.format("upper(%s)", x));
    }

    @Override
    public Predicate like(Expression<String> x, String pattern) {
        return new FakePredicate(x + " like " + pattern);
    }

    @Override
    public Predicate and(Expression<Boolean> x, Expression<Boolean> y) {
        return new FakePredicate(x + " and " + y);
    }

    @Override
    public Predicate or(Expression<Boolean> x, Expression<Boolean> y) {
        return new FakePredicate(x + " or " + y);
    }

    @Override
    public <Y extends Comparable<? super Y>> Predicate greaterThan(Expression<? extends Y> x, Y y) {
        return new FakePredicate(x + " > " + y);
    }
    ...
    public <X extends Comparable<? super X>> Expression<X> greatest(Expression<X> x) {
        return new FakeRoot<>(String.format("max(%s)", x));
    }
